...
const FooterToolsInitiator = {
  async init({subscribeButton, unsubscribeButton}) {
    ...
    this._regServiceWorker = null;

    if ('serviceWorker' in navigator) {
      this._regServiceWorker = await navigator.serviceWorker.getRegistration();
    };
  ...
  async _initialListener() {
    ...
    this._unsubscribeButton.addEventListener('click', (event) => {
      this._unsubscribePushMesssage(event);
    });
  },
  ...
  async _subscribePushMessage(event) {
    ...
    if (!(await this._isNotificationReady())) {
      console.log(`Notificiation isn't available`);
      return;
    }

    console.log('subscribePushMessage: subscribing to push message...');
    const pushSubscription = await this._regServiceWorker?.pushManager.subscribe(
  ...
  async _unsubscribePushMessage(event) {
    event.stopPropagation();
    const pushSubscription = await this._regServiceWorker?.pushManager.getSubscription();
    if (!pushSubscription) {
    ...
    try {
      ...
      console.log('Failed to erase push notification data from server: ', err.message);
    }
    console.log('unsubscribePushMessage');
  },
  ...
  async _isCurrentSubscriptionAvailable() {
    const checkSubscription = await this._regServiceWorker?.pushManager.getSubscription();
